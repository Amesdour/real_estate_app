...
export default async function AdminPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      ...
      <p className="mt-1 text-sm text-brand-700 dark:text-honey-white">Platform-wide overview.</p>
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3">
        {stats.map((s) => (
          <Link key={s.label} href={s.href} className="card card-interactive block p-4">
            ...
            <p className="mt-1 text-xs text-brand-700 dark:text-honey-white">{s.label}</p>
          </Link>
        ))}
      </div>
